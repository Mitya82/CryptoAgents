...
def fetch_ohlcv(exchange: Any, symbol: str, timeframe: str = "1h", limit: int = 100) -> List[List]:
    ...
    logging.debug(f"Загружено {len(candles)} свечей для {symbol} на {timeframe}")
    return candles


def calculate_sma(candles: List[List], period: int = 20, price_index: int = 4) -> Optional[float]:
    """Вычислить простую скользящую среднюю (SMA)."""
    if len(candles) < period:
        logging.warning(f"Недостаточно данных для SMA({period}): есть только {len(candles)} свечей")
        return None
...
def calculate_ema(candles: List[List], period: int = 12, price_index: int = 4) -> Optional[float]:
    """Вычислить экспоненциальную скользящую среднюю (EMA)."""
    if len(candles) < period:
        logging.warning(f"Недостаточно данных для EMA({period}): есть только {len(candles)} свечей")
        return None
...
def determine_trend(candles: List[List], fast_period: int = 9, slow_period: int = 21) -> str:
...
def check_pinbar_buy_signal(previous_candle: List, trend_direction: str = "uptrend") -> bool:
    """
    ...
    if lower_shadow >= (body_size / 2):
        logging.info(f"[ПИН-БАР] Тело: {body_size:.8f}, Нижняя тень: {lower_shadow:.8f}")
        return True
    
    return False


def check_pinbar_sell_signal(previous_candle: List, trend_direction: str = "downtrend") -> bool:
    ...
    if upper_shadow >= (body_size / 2):
        logging.info(f"[ПИН-БАР ПРОДАЖА] Тело: {body_size:.8f}, Верхняя тень: {upper_shadow:.8f}")
        return True
    
    return False


def place_buy_order(exchange: Any, symbol: str, amount: float, price: Optional[float] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.info(f"✅ ОРДЕР НА ПОКУПКУ: {symbol} x{amount} @ {price or 'MARKET'}")
...
def place_sell_order(exchange: Any, symbol: str, amount: float, price: Optional[float] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.info(f"✅ ОРДЕР НА ПРОДАЖУ: {symbol} x{amount} @ {price or 'MARKET'}")
        logging.info(f"Order ID: {order.get('id')}")
        return order
    except Exception as e:
...
def run_trading_loop(
    ...
    logging.info(f"Биржа: {exchange_id.upper()}, Пара: {symbol}, TF: {timeframe}")
    logging.info(f"Режим: {'ДЕМО' if demo_mode else 'РЕАЛЬНАЯ ТОРГОВЛЯ'}")
    logging.info("=" * 60)
    
    try:
        exchange = create_exchange(exchange_id)
        logging.info(f"✅ Подключено к: {getattr(exchange, 'name', exchange_id)}")
        ...
        while True:
            ...
            try:
                ...
                logging.info(f"\n[{iteration}] {time_str} | Тренд: {trend.upper()} | Цена: ${current_close:.2f}")
                
                # Проверяем сигналы только на предыдущей закрытой свече
                if current_time != last_signal_candle_time and len(candles) >= 2:
...
def main() -> None:
    ...
    parser.add_argument("--timeframe", "-tf", default="1h", help="Candle timeframe (default: 1h)")
    parser.add_argument("--interval", "-i", type=int, default=60, help="Check interval in seconds (default: 60)")
    parser.add_argument("--amount", "-a", type=float, default=0.001, help="Trade amount (default: 0.001)")
    parser.add_argument("--trade", action="store_true", help="Enable real trading (default: demo mode)")
